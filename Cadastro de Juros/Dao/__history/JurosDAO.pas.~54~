...
begin
  try
    ...
    try
      sSQL :=
      'insert into juros (juros, jurosmora, acrescimo, tipojurosid, diaatraso, statusgrupoid, identifier) '+
      ...
      ' '+IntToStr(juros.getDiaAtraso)+', '+IntToStr(juros.getStatusGrupo)+', 34107328000123) ';
...
function TJurosDao.existeCadastroTipoJuros(TipoJuros: TTipoJuros): boolean;
var
  zGet: TZQuery;
  sSQL: string;
  bExiste: boolean;
begin
  try
    ...
    sSQL :=
    'select id from juros where tipojurosid = '+IntToStr(TipoJuros.getCodigo)+' ';
...
begin
  try
   ...
   zGet.Connection := dmPrincipal.ZConnection;
    ...
    'statusgrupo.descricao, juros.statusgrupoid, juros.identifier '+
    'from juros '+
...
procedure TJurosDao.pesquisarStringGrid(zGet: TZQuery);
var
  sSQL: string;
begin
  try
    zGet.Connection := dmPrincipal.ZConnection;
    sSQL := 'select juros.id as codigo, cast(upper(tipojuros.descricao) as varchar(30)) as tipojuros, '+
            'juros.juros, juros.jurosmora, juros.acrescimo, juros.diaatraso, '+
            'statusgrupo.descricao, tipojuros.id as CodTipoJuros '+
            'from juros join tipojuros on juros.tipojurosid = tipojuros.id '+
            'join statusgrupo on statusgrupo.id = juros.statusgrupoid '+
            'where statusgrupo.grupostatusid = 6';
