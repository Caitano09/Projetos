...
begin
  ...
  begin
    ...
    if (gdSelected in State) or (gdFocused in State) or (DBGrid1.SelectedRows.CurrentRowSelected) then
...
begin
  ...
  ProgressBar1.Max := StrToInt(lblNumRegistros.Caption);
  zqGet.First;
  while not zqGet.Eof do
  begin
    xLinhaCompleta := zqGet.FieldByName('juros').AsString + xCaracter +
                      ...
                      zqGet.FieldByName('statusgrupoid').AsString + xCaracter +
                      zqGet.FieldByName('identifier').AsString;
...
begin
  ...
  begin
    ...
    begin
      sql.Clear;
      sql.Add('insert into juros (juros, jurosmora, acrescimo, diaatraso, tipojurosid, statusgrupoid, identifier) '+
      ...
      ' '+ValidarPesquisa.VirgulaPonto(quebra[4])+', '+ValidarPesquisa.VirgulaPonto(quebra[5])+', '+ValidarPesquisa.VirgulaPonto(quebra[6])+' ) ');
